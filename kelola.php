            ...
            <div class="col-lg-8">
                <div class="card shadow border-0 rounded-4">
                    <div class="card-header bg-primary text-white rounded-top-4">
                        <h4 class="mb-0"><i class="bi bi-person-plus-fill me-2"></i>Tambah Data Pelanggan</h4>
                    </div>
                    <div class="card-body p-4 bg-light rounded-bottom-4">
                    <form action="proses.php?tambah=pelanggan" method="post" class="needs-validation" novalidate>
                <!-- ID Pelanggan -->
                    <div class="mb-3">
                    <label for="idPelanggan" class="form-label">ID Pelanggan</label>
                        <input type="text" class="form-control bg-light border-0 text-muted fw-semibold" name="idPelanggan" id="idPelanggan" value="<?= $newIdPelanggan ?>" readonly>
                    </div>
                ...
                <!-- Jenis Kelamin -->
                    ...
                    <input class="form-check-input" type="radio" name="jenis_kelamin" value="Pria" id="pria" required>
                ...
                <div class="mb-3">
                    ...
                    <input type="tel" class="form-control" name="nomorTelepon" id="nomorTelepon" required>
                </div>
...
<!-- akhir form input pelanggan-->
        ...
        <div class="container d-flex justify-content-center align-items-center my-5">
            <div class="col-lg-8">
                <div class="card shadow border-0 rounded-4">
                    <div class="card-header bg-primary text-white rounded-top-4">
                        <h4 class="mb-0"><i class="bi bi-tools me-2"></i>Tambah Jenis Jasa Service</h4>
                    </div>
                    <div class="card-body p-4 bg-light rounded-bottom-4">
                        <form action="proses.php?tambah=servis" method="post" class="needs-validation" novalidate>
                            <!-- ID Service -->
                            <div class="mb-3">
                                <label for="idService" class="form-label">ID Service</label>
                                <input type="text" class="form-control bg-light border-0 text-muted fw-semibold" name="idService" id="idService" value="<?= $newIdService ?>" readonly>
                            </div>

                            <!-- Jenis Service -->
                            <div class="mb-3">
                                <label for="jenisService" class="form-label">Jenis Service</label>
                                <input type="text" class="form-control" name="jenisService" id="jenisService" required>
                            </div>

                            <!-- Biaya Service -->
                            <div class="mb-4">
                                <label for="biayaService" class="form-label">Biaya Service (Rp)</label>
                                <input type="number" class="form-control" name="biayaService" id="biayaService" required>
                            </div>
        ...
        <?php elseif (isset($_GET['tambah']) && $_GET['tambah'] == 'sparepart'): ?>
            ...
            <div class="col-lg-8">
                <div class="card shadow border-0 rounded-4">
                    <div class="card-header bg-primary text-white rounded-top-4">
                        <h4 class="mb-0"><i class="bi bi-cpu-fill me-2"></i>Tambah Data Sparepart</h4>
                    </div>
                    <div class="card-body bg-light rounded-bottom-4">
                        <form action="proses.php?tambah=sparepart" method="post" class="needs-validation" novalidate>

                            <!-- Kode Barang (Auto ID) -->
                            <div class="mb-3">
                                <label for="kodeBarang" class="form-label">Kode Barang</label>
                                <input type="text" class="form-control bg-light border-0 text-muted fw-semibold" 
                                    name="kodeBarang" id="kodeBarang" value="<?= $newKodeBarang ?>" readonly>
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control" name="merkBarang" id="merkBarang" required>
                            </div>
            ...
            <?php elseif (
                (isset($_GET['tambah']) && $_GET['tambah'] == 'transaksi') || 
                (isset($_GET['tombol']) && $_GET['tombol'] == 'tambahTransaksi')
            ): ?>
            <h2 class="fw-bold text-center">Tambah Data Transaksi</h2>
            <p class="text-center text-muted">Formulir untuk menginput data transaksi<br><em>— CRUD: Create, Read, Update, Delete</em></p>
            ...
            <div class="container my-5">
                <div class="col-lg-8 mx-auto">
                    <div class="card shadow-sm border-0 rounded-4">
                        <div class="card-header bg-primary text-white rounded-top-4">
                            <h4 class="mb-0"><i class="bi bi-receipt-cutoff me-2"></i>Tambah Data Transaksi</h4>
                        </div>
                        <div class="card-body bg-light rounded-bottom-4">
                            <form action="proses.php?tambah=transaksi" method="post">
                                <input type="hidden" name="aksi" value="tambah">
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="text" class="form-control bg-light border-0 text-muted fw-semibold" name="id_transaksi" value="<?= $id_transaksi ?>" readonly>
                                </div>
                                ...
                                <div class="mb-3">
                                    ...
                                    <select class="form-select" name="id_service" required>
                                        ...
                                        <?php while ($row = mysqli_fetch_assoc($service)): ?>
                                            <option value="<?= $row['id_service'] ?>"><?= $row['jenis_service'] ?></option>
                                        <?php endwhile; ?>
                                    </select>
                                </div>
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="date" class="form-control" name="tanggal_masuk" value="<?= $tanggal_masuk ?>">
                                </div>

                                <!-- Kerusakan -->
                                <div class="mb-3">
                                    <label for="kerusakan" class="form-label">Kerusakan</label>
                                    <textarea class="form-control" name="kerusakan" required></textarea>
                                </div>
                                        ...
                                        <div class="input-group mb-2">
                                            <select name="sparepart[]" class="form-select me-2">
                                                <?php mysqli_data_seek($sparepart, 0); while ($s = mysqli_fetch_assoc($sparepart)) : ?>
                                                    <option value="<?= $s['kd_barang'] ?>"><?= $s['nama_barang'] ?> - Rp<?= $s['harga_barang'] ?></option>
                                                <?php endwhile; ?>
                                            </select>
                                            <input type="number" name="jumlah[]" class="form-control" placeholder="Jumlah">
                                        </div>
                                    </div>
                                    <button type="button" onclick="tambahSparepart()" class="btn btn-outline-secondary btn-sm mt-1">Tambah Sparepart</button>
                                </div>
